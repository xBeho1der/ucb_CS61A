...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in dices:
        if i == 1:
            SowSad = True
    return 1 if SowSad else sum(dices)
    # END PROBLEM 1
...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    tens, ones = (opponent_score % 100)//10, opponent_score % 10
...
# BEGIN PROBLEM 4
def perfect_square(score: int):
    ls = [i**2 for i in range(1, 1000)]
    return True if score in ls else False


def next_perfect_square(score: int):
    ls = [i**2 for i in range(1, 1000)]
    return ls[ls.index(score)+1] if score in ls else None
# END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    return True if str(strategy)[10:21] == "always_roll" else False
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    totals = [ma(i, dice) for i in range(1, 11)]
    return totals.index(max(totals))+1
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    bg = score
    score = tail_points(opponent_score)+score
    judge = perfect_square(score)
    if judge:
        score = (score**0.5+1)**2
    return 0 if score - bg > threshold else num_rolls
    # END PROBLEM 11
